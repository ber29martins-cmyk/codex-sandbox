...
### Auth bypass local (somente desenvolvimento)

Quando `KV_REST_API_URL`/`KV_REST_API_TOKEN` não estiverem configurados, é possível habilitar bypass de autenticação beta local com:

```bash
export BETA_AUTH_DEV_BYPASS=1
```
